...
function toListResult(
  ...
  if (Array.isArray(data)) {
    return {
      content: data.map(toFrontendProduct),
      total: data.length,
    };
  }
...
export const produtoService = {
  async listarProdutos(params: ProductListParams = {}) {
    const { data } = await api.get<
      BackendProduct[] | PageableResponse
    >("/produtos", {
      params,
    });
  ...
  async criarProduto(input: ProductInput) {
    ...
    const { data } = await api.post<BackendProduct>(
      "/produtos",
      payload
    );
  ...
  async editarProduto(id: string, input: ProductInput) {
    ...
    const { data } = await api.put<BackendProduct>(
      `/produtos/${id}`,
      payload
    );
